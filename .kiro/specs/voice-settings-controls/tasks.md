...
- [ ] 1. Add voice settings types, constants, and schema to the shared package
  - [ ] 1.1 Create `packages/shared/src/types/voice-settings.types.ts` with `VoiceSettings` interface, `VoiceSettingRange` interface, `VOICE_SETTINGS_RANGES` constant, and `DEFAULT_VOICE_SETTINGS` constant
  ...
  - [ ] 1.2 Add `voiceSettingsSchema` to `packages/shared/src/schemas/pipeline.schema.ts` and add optional `voiceSettings` field to `createPipelineJobSchema`
  ...
  - [ ] 1.3 Export new types, constants, and schema from `packages/shared/src/index.ts`
...
- [ ] 2. Checkpoint — Ensure shared package builds and all tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Add voice settings to the database and domain layer
  - [ ] 3.1 Add `voiceSettings Json?` column to the `PipelineJob` model in `apps/api/prisma/schema.prisma` and run Prisma migration
  ...
  - [ ] 3.2 Update `PipelineJob` domain entity in `apps/api/src/pipeline/domain/entities/pipeline-job.ts`
...
- [ ] 4. Update mapper and use cases to carry voice settings
  - [ ] 4.1 Update `PipelineJobMapper` in `apps/api/src/pipeline/infrastructure/mappers/pipeline-job.mapper.ts`
    - Import `VoiceSettings` from `@video-ai/shared`
  ...
  - [ ] 4.2 Update `CreatePipelineJobUseCase` in `apps/api/src/pipeline/application/use-cases/create-pipeline-job.use-case.ts`
  ...
  - [ ] 4.3 Update `GetJobStatusUseCase` and `PipelineJobDto` to include voice settings
...
- [ ] 5. Checkpoint — Ensure API builds and all tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Update TTS service and worker to use voice settings
  - [ ] 6.1 Update `TTSService` interface in `apps/api/src/pipeline/application/interfaces/tts-service.ts`
    - Import `VoiceSettings` from `@video-ai/shared`
    - Add `voiceSettings: VoiceSettings` to the `generateSpeech` params (required — worker applies default before calling)
    - _Requirements: 7.1, 7.2_

  - [ ] 6.2 Update `ElevenLabsTTSService` in `apps/api/src/pipeline/infrastructure/services/elevenlabs-tts-service.ts`
  ...
  - [ ] 6.3 Update `TTSGenerationWorker` in `apps/api/src/pipeline/infrastructure/queue/workers/tts-generation.worker.ts`
...
- [ ] 7. Checkpoint — Ensure API builds and all backend tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 8. Build the frontend VoiceSettingsControls component
  - [ ] 8.1 Create `apps/web/src/features/pipeline/components/voice-settings-controls.tsx`
...
- [ ] 9. Wire VoiceSettingsControls into DraftHero and PipelineWizard
  - [ ] 9.1 Update `CreateJobParams` in `apps/web/src/features/pipeline/interfaces/pipeline-repository.ts`
    - Import `VoiceSettings` from `@video-ai/shared`
    - Add `voiceSettings?: VoiceSettings` to `CreateJobParams`
    - _Requirements: 5.1_

  - [ ] 9.2 Update `DraftHero` in `apps/web/src/features/pipeline/components/draft-hero.tsx`
  ...
  - [ ] 9.3 Update `PipelineWizard` in `apps/web/src/features/pipeline/components/pipeline-wizard.tsx`
    - Import `DEFAULT_VOICE_SETTINGS` from `@video-ai/shared` and `VoiceSettingsControls`
    - Add `voiceSettings` state initialized to `DEFAULT_VOICE_SETTINGS`
    - Render `<VoiceSettingsControls>` directly below the `<VoiceSelector>` section
...
- [ ] 10. Final checkpoint — Ensure full build passes and all tests pass
  - Ensure all tests pass, ask the user if questions arise.
