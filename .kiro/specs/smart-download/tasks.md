...
- [ ] 1. Add `computeCodeHash` utility and database column
  - [ ] 1.1 Create `computeCodeHash` domain service
  ...
  - [ ] 1.3 Add `lastRenderedCodeHash` column to Prisma schema
...
- [ ] 2. Extend `PipelineJob` entity and mapper
  - [ ] 2.1 Add `lastRenderedCodeHash` to `PipelineJob` entity
  ...
  - [ ] 2.2 Update `PipelineJobMapper` for `lastRenderedCodeHash`
...
- [ ] 4. Store hash at render completion and expose `codeChanged` in DTO
  - [ ] 4.1 Update `VideoRenderingWorker` to compute and store hash
...
- [ ] 6. Implement `SmartDownloadButton` frontend component
  - [ ] 6.1 Create `SmartDownloadButton` component
...
- [ ] 7. Integrate smart download into `VideoPreviewPage`
  - [ ] 7.1 Replace existing buttons with `SmartDownloadButton`
  ...
  - [ ] 7.2 Add auto-download after render completes
