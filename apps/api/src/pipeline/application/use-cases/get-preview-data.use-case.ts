import type { ScenePlan, VideoFormat as VideoFormatType } from "@video-ai/shared";
...
export class GetPreviewDataUseCase
  implements UseCase<GetPreviewDataRequest, Result<PreviewDataResponse, ValidationError>>
{
  constructor(
  ...
  async execute(
    ...
    if (!VALID_PREVIEW_STAGES.includes(stage as (typeof VALID_PREVIEW_STAGES)[number])) {
      return Result.fail(
        new ValidationError(
    ...
    if (!code) {
      return Result.fail(
        new ValidationError(`Job "${request.jobId}" has no generated code`, "NOT_FOUND"),
      );
    }
    ...
    if (!sceneDirections) {
      return Result.fail(
        new ValidationError(`Job "${request.jobId}" has no scene directions`, "NOT_FOUND"),
      );
    }
    ...
    if (!transcript || transcript.length === 0) {
      return Result.fail(
        new ValidationError(`Job "${request.jobId}" has no transcript`, "NOT_FOUND"),
      );
    }
    ...
    if (!theme) {
      return Result.fail(
        new ValidationError(`Animation theme not found: ${job.themeId.value}`, "NOT_FOUND"),
      );
    }
